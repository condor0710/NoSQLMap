...
def save_to(savePath, vulnAddrs, possAddrs, strTbAttack,intTbAttack):
    fo = open(savePath, "wb")
    fo.write (b"Vulnerable URLs:\n")
    fo.write("\n".join(vulnAddrs).encode())
    fo.write(b"\n\n")
    fo.write(b"Possibly Vulnerable URLs:\n")
    fo.write("\n".join(possAddrs).encode())
    fo.write(b"\n")
    fo.write(b"Timing based attacks:\n")
    
    if strTbAttack == True:
        fo.write(b"String Attack-Successful\n")
    else:
        fo.write(b"String Attack-Unsuccessful\n")
    fo.write(b"\n")
    
    if intTbAttack == True:
        fo.write(b"Integer attack-Successful\n")
    else:
        fo.write(b"Integer attack-Unsuccessful\n")
    fo.write(b"\n")
    fo.close()
...
def postApps(victim,webPort,uri,https,verb,postData,requestHeaders, args = None):
    ...
    if appUp == True:
        ...
        del gtDict[injOpt]
        body = urllib.parse.urlencode(gtDict).encode()
        req = urllib.request.Request(appURL,body, requestHeaders)
        if verb == "ON":
...
def errorTest (errorCheck,testNum):
    ...
    if errorCheck.find(b'ReferenceError') != -1 or errorCheck.find(b'SyntaxError') != -1 or errorCheck.find(b'ILLEGAL') != -1:
